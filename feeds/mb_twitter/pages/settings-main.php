...
global $mb_globalfeed,$mb_globalfeed_twitter_connect,$mbgfa_theme;

$update = false;

if (isset($_POST['_update'])){
    if (!(check_admin_referer( 'twitter-connect-admin' ) && wp_verify_nonce( $_POST['_wpnonce'], 'twitter-connect-admin' )))
            die('Security check failed.');

    // An update has been submitted. Save the settings.
    $update = true;

    $feed_options['allow_showing_retweets'] = isset($_POST['general-allow_showing_retweets']) && $_POST['general-allow_showing_retweets'] == 'on';
...
<form action="<?php $_SERVER['REQUEST_URI'] ?>" method="post">
    <?php if (function_exists('wp_nonce_field')){
        echo wp_nonce_field('twitter-connect-admin');
    } ?>
    <div id="notices">
        <?php  // Show welcome Message
        if ( $feed_options['show_welcome_message'] ) :
            //$feed_options['show_welcome_message'] = false;
        ?>
            <div class="notice updated no-hide" ><?php _e('<p>Welcome to MB Twitter. You\'ll find the plugins\' main configuration here.</p><p>Please report any bugs you find by using the bug report feature at the top right of the page. Thank you for using MB GlobalFeed!</p>') ?></div>
    ...
    <!-- Twitter Object Information  -->
    <fieldset id="tw_object_information" class="ajax_option optgrp"><legend><?php _e( 'Twitter User Information' , $mb_globalfeed_twitter_connect->get_slug() ) ?></legend>
        <div class="leftopt">
            <?php $mbgfa_theme->ajaxIndicator(true, 'center'); ?>
        </div>
    </fieldset>
    <fieldset id="tw_general_options" class="optgrp"><legend>General Options</legend>
        <table>
            <p><label for="general-allow_showing_retweets"><input type="checkbox" <?php echo $feed_options['allow_showing_retweets'] ? 'checked="checked"' : ''; ?> id="general-allow_showing_retweets" name="general-allow_showing_retweets" /> Allow importing and showing of retweets</label></p>
            <p>Note: If you are encountering an issue wherein tweets are showing up late with an incorrect date/time and/or WordPress is marking posts as "future", enabling the option below may fix any timezone handling problems.</p>
    ...
    <!-- Reset Options -->
    <fieldset id="tw_reset_options" class="optgrp"><legend>Reset Feed</legend>
        <table>
            ...
            <tr id="redo_initial_setup_cont"><td><input type="button" value="Redo Initial Setup" id="redo_initial_setup" /></td><td><?php $mbgfa_theme->informative('This will bring up the initial setup wizard and remove all stored feed items, however it will not reset any settings.'); ?><?php $mbgfa_theme->ajaxIndicator(true); ?></td></tr>
            <tr id="reset_feed_defaults_cont"><td><input type="button" value="Reset Feed Defaults" id="reset_feed_defaults" /></td><td><?php $mbgfa_theme->informative('This will completely reset the feed to its initial settings, simultaneously removing all stored feed items.'); ?><?php $mbgfa_theme->ajaxIndicator(true); ?></td></tr>
        </table>
    </fieldset>
</form>
    <script type="text/javascript">
        jQuery(document).ready(function(){
            ...
            get_tw_object( '<?php echo $feed_options['object_to_subscribe'] ?>' ,'#tw_object_information .leftopt' );
            ...
            jQuery('#redo_initial_setup').click(function(){
                ...
                jQuery.ajax({
                    url: ajaxurl,
                    data: {action:'mbgf_twitter_connect_redo_initial_setup',_wpnonce:jQuery('#_wpnonce').val()},
                    statusCode: { 
            ...
            jQuery('#reset_feed_defaults').click(function(){
                ...
                jQuery.ajax({
                    url: ajaxurl,
                    data: {action:'mbgf_twitter_connect_reset_feed_defaults',_wpnonce:jQuery('#_wpnonce').val()},
                    statusCode: { 
                        200: function (response) {
                            window.location.reload();
                        }},
            ...
            jQuery('#manual_update').click(function(){
                ...
                jQuery.ajax({
                    url: ajaxurl,
                    data: {action:'mbgf_twitter_connect_manual_feed_update',_wpnonce:jQuery('#_wpnonce').val()},
                    statusCode: { 
                        200: function (response) {
